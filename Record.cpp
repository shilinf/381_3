...
Record::Record(const string& medium_, const string& title_) : ID(++ID_counter),
            rate(0), title(title_), medium(medium_) {}

Record::Record(const string& title_) : ID(0), rate(0), title(title_) {}

Record::Record(int ID_) : ID(ID_), rate(0) {}

Record::Record(const Record &old, const std::string& title_) : ID(old.ID),
                rate(old.rate), title(title_), medium(old.medium) {}
...
{
    if (record.rate)
        os << record.ID << ": " << record.medium << " " << record.rate << " "
            << record.title;
    else
